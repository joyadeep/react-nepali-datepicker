...
import previmg from "../../assets/left-arrow.svg";
import nextimg from "../../assets/right-arrow.svg";
...
const Header = ({ year, month, setyear, setmonth }) => {
  ...
  return (
    <div className="picker-header-container">
      <div
        className={`prev-btn ${year <= 2e3 ? "hide" : ""}`}
        onClick={() => {
          if (month == 0) {
            setmonth(11);
            setyear(year - 1);
      ...
      <div className="year-select">
        <Select options={years} year value={year} setValue={setyear} />
      </div>
      <div
        className={`next-btn ${year >= 2090 ? "hide" : ""}`}
        onClick={() => {
          if (month == 11) {
            setmonth(0);
            setyear(year + 1);
